...
Status: implementation complete (2026-09-17) — Phases 0, 1a, 1, 2, 3, 4, 5 all implemented; the plan's falsifiable perf gate (≥55fps on a named low-end device) awaits its manual real-device verification — automated proxy numbers below · rev 2 (2026-09-17) — amended after adversarial review with code verification and external fact-checking · Scope: rendering/UX layer. The engine (`src/engine/`) is untouched; the store (`src/store/`) receives **additive** changes (result plumbing, `reducedMotion` setting — see [Store & data plumbing](#store--data-plumbing)); persistence shape, routing, and i18n architecture are otherwise untouched.
...
### Phase 4 — Hardening: implemented (2026-09-17)

- **Texture loading:** the mount deal gates on the room's ≤4 card textures (≤1.5s fallback so a stalled load can never freeze the board); placeholder→texture swaps stay seam-safe mid-unfold. Only the current room's textures load — no 44-request waterfall, no atlas (the 44 hashed JPEGs are already browser-cached and shared with the DOM fallback; an atlas would duplicate them per the plan's own caveat).
- **Scale robustness:** a parent-ResizeObserver + silent-adopt path handles room-box changes that aren't window resizes (the canvas-live flip on wrapped boards); same-size RESIZE events are ignored so boot adoption can't kill a starting deal; a canvas-box self-heal invariant (canvas must match its parent, else `scale.refresh()`) fixed a real bug — mobile 2×2 canvas frozen 9.4px taller than its room (−9.4 → 0.59px after heal). Real resizes mid-beat still snap (verified 1280→1000).
- **Determinism audit:** zero `Math.random`/engine-RNG usage in `src/game/` — all choreography derives from the sync diff; FX randomness is Phaser-internal and cosmetic only.
- **Perf gate (falsifiable) — proxy numbers, NOT device-verified:** headless-Chromium rAF-delta sampling of pure choreography windows (page-load work excluded): deal ≈ 47–52fps avg sustained, kill beat ≈ 51–53fps avg; worst frames 12–20 (66–83ms) concentrated at load/texture boundaries; 6× CPU throttle ≈ 51–52fps, 12× ≈ 41–47. These are **instrument-limited** — headless rAF is schedule-capped and cannot certify the 55fps gate; **the gate awaits its manual run**: Chrome DevTools → Performance trace on an iPhone 11 / Moto G-class device (or DevTools CPU 6× + 4× network as the desktop proxy), during a fresh deal + one full resolve sequence. `reducedMotion` users never see sustained animation (opacity-only), so the gate applies to full-motion users only.
- **Bundle:** CI budget step added (`scripts/check-bundle-size.mjs`, node:zlib, per-chunk ≤ 420 KB gz / total ≤ 550 KB gz — the plan's ≤ +400 KB gz `#/play` delta with documented headroom); real dist: phaser.esm 372.7, main 95.8, scene 5.7 → total 474.5 KB gz. Passes.

### Phase 5 — Test & CI updates: implemented (2026-09-17)

- 17 Phaser-free unit tests for the bridge (fan-out identity, unsubscribe, latest-sync replay, transient hover, one-shot sceneReady contract, mid-emit subscribe/unsubscribe semantics, idempotent destroy) — 127 unit/integration tests total.
- `data-canvas-ready` probe on the board container (fires with the scene's first reconcile); `e2e/board-parity.spec.ts` waits on it — precise, race-free. Rect assertions at both viewports, axe scans (WCAG 2.1/2.2 A+AA, zero violations), resume-silence smoke — all in e2e, all running in CI.
- `README.md` architecture section documents the Phaser layer and the hit-layer pattern; this file's status tracks implementation state.
